...
$.extend(Popup.prototype, {
    ...
    __center: function () {
        ...
        var oh = popup.height();
        var left = (ww - ow) / 2 + dl;
        var top = (Math.min(wh, dh) - oh) / 2 + dt;// 黄金比例
    ...
    __lock: function () {
        ...
        backdrop.css(backdropCss).animate({opacity: this.backdropOpacity}, 150)
        .insertAfter(popup)
        // 锁住模态对话框的 tab 简单办法
    ...
    button: function (args) {
        ...
        if (typeof args === 'string') {
            ...
            $.each(args, function (i, val) {
                ...
                + ' data-id="' + val.id + '"' + style + (val.disabled ? ' disabled' : '')
    ...
    if (url) {
        ...
        .on('load', function () {
            ...
            if (test) {
                ...
                var docHeight = $($iframe[0].contentWindow.document.body).height();
                if (!options.height) {
                	$iframe.height(docHeight);
                    api.height(docHeight);
                }
            }
